...
df_affiliations_grouped <- df_affiliations_wrangled %>%
  ...
  summarise(n_aff_contracts = sum(!is.na(Partnership__c)),
            n_aff_accounts = sum(is.na(Partnership__c)),
    
            all_contracts = paste0(Partnership__c[aff_type=='Contract'], collapse = ','),
            
            all_isds = paste0(isd[aff_type=='ISD/RESA/BOCES'], collapse = ','),
            ISD_RESA_BOCES__c = paste0(aff_entity_code[aff_type=='ISD/RESA/BOCES'], collapse = ','),
            
            all_districts = paste0(district[aff_type=='District'], collapse = ','),
            District__c = paste0(aff_entity_code[aff_type=='District'], collapse = ','),
            
            all_schools = paste0(school[aff_type=='School'], collapse = ','),
            School__c = paste0(aff_entity_code[aff_type=='School'], collapse = ','),
            
            all_implementing_schools = paste0(implementing_school[aff_type=='Implementing School'], collapse = ','),
            
            all_orgs = paste0(isd[aff_type=='Org/Agency'], collapse = ','),
            Org_Agency__c = paste0(aff_entity_code[aff_type=='Org/Agency'], collapse = ',')
  )
...
df_out_missing_both_affiliations <- df_sf_training_attendances_wrangled %>%
  select(-c(Country__c, State__c, Partnership__c, ISDs, District__c, School__c, Implementing_Schools__c, Org_Agency__c)) %>%
  mutate(
  ...
  anti_join(x=., y=df_affiliations_grouped['attendance_id'],
            ...
            na_matches='never')  %>%
  left_join(x=., 
            y=df_trainings_reach_wrangled[c('Email__c', 'isd_name_reach', 'ISD_RESA_BOCES__c',
                                            'district_name_reach', 'District__c',
                                            'school_name_reach', 'School__c', 'org_name_reach', 'Org_Agency__c', 'Partnership__c', 'reach_notes',
                                            'n_trainings', 'n_affiliations', 'n_contracts', 'flag_multiple_affiliations', 
                                            'flag_multiple_contracts', 'flag_missing_affiliations', 'flag_missing_contracts')],
            by='Email__c', na_matches='never') %>%
  distinct() %>%
  rename(ISD_RESA_BOCES__c_reach='ISD_RESA_BOCES__c', 
...
df_sf_missing_one_affiliation <- df_sf_training_attendances_wrangled %>%
  ...
  filter((n_aff_contracts==0 |
          ...
          # Exclude records under community contract
           !grepl(pattern = 'TRAILS Free T3 Community--TRAILS Free Opportunity', x = all_contracts)
         ) 
```
